public class CheckIfMoveIsLegal {
    public boolean checkMove(char[][] board, int rMove, int cMove, char color) {
        ...
        char colorOposite = color=='W' ? 'B':'W';

        for (int i=rMove-1;i<=rMove+1;i++)
            if (i>=0 && i<8)
                for (int j=cMove-1;j<=cMove+1;j++)
                    if (j>=0 && j<8 && board[i][j]==colorOposite) {
                        int iParcurgere=i;
                        int jParcurgere=j;
                        while (iParcurgere>=0 && iParcurgere<8 && jParcurgere>=0 && jParcurgere<8 && board[iParcurgere][jParcurgere]==colorOposite)
                        {
                            iParcurgere=iParcurgere+i-rMove;
                            jParcurgere=jParcurgere+j-cMove;
                        }
                        if (iParcurgere>=0 && iParcurgere<8 && jParcurgere>=0 && jParcurgere<8 && board[iParcurgere][jParcurgere]==color)
                            return true;
                    }
        return false;
    }
